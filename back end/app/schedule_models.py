...
class AdherenceReportRequest(BaseModel):
    ...
    notes: str | None = None
    alert_level: str = "none"  # none | watch | urgent
...
class AdherenceReportRow(SQLModel, table=True):
    ...
    id: str = Field(
        default_factory=lambda: f"rep_{uuid4().hex[:16]}", primary_key=True, index=True
    )
    user_id: str = Field(index=True)
    ...
    created_at: str = Field(
        default_factory=lambda: datetime.now(timezone.utc).isoformat(), index=True
    )
