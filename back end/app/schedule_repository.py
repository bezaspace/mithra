...
class ScheduleRepository:
    ...
    def initialize(self) -> None:
        ...
        if not self._seed_sql_path.exists():
            logger.warning(
                "schedule_seed_missing seed_sql_path=%s", self._seed_sql_path
            )
            return
    ...
    def list_active_items(self, user_id: str) -> list[ScheduleItem]:
        ...
        with Session(self._engine) as session:
            rows = session.exec(
                ...
                .order_by(
                    ScheduleItemRow.display_order,
                    ScheduleItemRow.window_start_local,
                    ScheduleItemRow.id,
                )
            ).all()
            return [
                item for row in rows if (item := self._row_to_item(row)) is not None
            ]
    ...
    def list_reports_by_date(
        self, user_id: str, report_date_local: str
        ...
        with Session(self._engine) as session:
            rows = session.exec(
                ...
                .order_by(
                    AdherenceReportRow.reported_at_iso,
                    AdherenceReportRow.created_at,
                    AdherenceReportRow.id,
                )
            ).all()
            return [
                report
                for row in rows
                if (report := self._row_to_report(row)) is not None
            ]
    ...
    def list_reports_for_item(
        ...
        with Session(self._engine) as session:
            ...
            if report_date_local:
                query = query.where(
                    AdherenceReportRow.report_date_local == report_date_local.strip()
                )
            rows = session.exec(
                query.order_by(
                    AdherenceReportRow.reported_at_iso, AdherenceReportRow.created_at
                )
            ).all()
            return [
                report
        ...
        *,
        ...
        with Session(self._engine) as session:
            row = session.exec(
                ...
                .where(
                    AdherenceReportRow.conversation_turn_id
                    == conversation_turn_id.strip()
                )
            ).first()
            if row is None:
                return None
    ...
    def _row_to_item(self, row: ScheduleItemRow) -> ScheduleItem | None:
        try:
            instructions = self._decode_list(
                row.instructions_json, field_name="instructions_json", row_id=row.id
            )
        except ValueError:
            return None
        try:
            ...
            logger.warning(
                "schedule_item_invalid_shape id=%s error_type=%s",
                row.id,
                type(exc).__name__,
            )
            return None
    ...
    def _row_to_report(self, row: AdherenceReportRow) -> AdherenceReport | None:
        try:
            ...
            logger.warning(
                "adherence_report_invalid_shape id=%s error_type=%s",
                row.id,
                type(exc).__name__,
            )
            return None
    ...
    def _decode_list(raw: str, *, field_name: str, row_id: str) -> list[Any]:
        try:
            ...
            logger.warning(
                "schedule_json_decode_failed id=%s field=%s", row_id, field_name
            )
